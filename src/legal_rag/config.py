...
class Settings(BaseSettings):
    ...
    # Can be configured using LEGAL_RAG_GENERATION_MAX_TOKENS.
    generation_max_tokens: int = Field(default=1024, ge=1, le=32768)

    generation_retry_count: int = Field(default=1024, ge=0, le=3)
    maximum_context_characters: int = Field(
        default=1_024,
        ge=1_024,
        le=32_768,
    )
    ...
    experimental_identifier_override_score: float = Field(
        default=0.75,
        ge=-1,
        le=1,
    )
    experimental_min_rerank_score: float | None = -1.0
    experimental_max_dense_score_drop: float = Field(
        default=0.02,
        ge=0,
        le=2,
    )
