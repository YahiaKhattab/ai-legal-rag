...
_ARABIC_ARTICLE = re.compile(
    r"^(?P<label>المادة|مادة)\s*(?:\([^)]{1,30}\)|\S{1,30})?"
)
_ENGLISH_SECTION = re.compile(
    r"^(?P<label>part|chapter|section)\b",
    re.IGNORECASE,
)
...
def _correct_reversed_arabic_article_heading(line: str) -> str:
    """Correct OCR-reversed Arabic article numbers and parentheses.

    Example:
        مادة )٩٤١( -> مادة (١٤٩)
    """
    match = re.match(
        r"^(?P<label>المادة|مادة)\s*(?P<rest>.*)$",
        line,
    )
    if not match:
        return line
    ...
    # OCR may reverse both the parentheses and the digits.
    rest = re.sub(
        r"\)([٠-٩]{1,30})\(",
        lambda number_match: f"({number_match.group(1)[::-1]})",
        rest,
        count=1,
    )

    return f"{label}{' ' if rest else ''}{rest}".strip()
...
def classify_line(line: str) -> LineClassification:
    ...
    # Fix OCR-reversed Arabic article headings before classification.
    normalized = _correct_reversed_arabic_article_heading(normalized)
...
class LegalStructureDetector:
    ...
    def detect_source(self, source: SourceRecord) -> list[SectionSpan]:
        if (
            source.extraction_method is ExtractionMethod.FAILED
            or not source.original_text.strip()
        ):
            return []
